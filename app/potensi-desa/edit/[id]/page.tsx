...
export default function EditPotensiDesa() {
  ...
  useEffect(() => {
    const loadPotensiDesa = async () => {
      setIsLoading(true);
      try {
        const response = await fetchData(`/potensi-desa/getPotensiDesa/${id}`);
        console.log("API Response:", response); // Debug log
        const data: PotensiDesa = response.potensiDesa;
        setNama(data.nama);
        setDeskripsi(data.deskripsi === "undefined" ? "" : data.deskripsi); // Handle "undefined" string
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await fetchData(`/potensi-desa/updatePotensiDesa/${id}`, {
        method: "PUT",
      ...
      Swal.fire({
        icon: "success",
        title: "Berhasil",
        text: "Potensi desa berhasil diperbarui!",
      });
