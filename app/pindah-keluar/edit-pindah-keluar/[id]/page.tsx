...
export default function EditPindahKeluarPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Load existing PindahKeluar data
        const response = await fetchData(`/mutasi/getPindahKeluar/${id}`);
        const data = response.pindahKeluar || response;
        setFormData({
          nama: data.nama || "",
          nik: data.nik || "",
          tanggal_pindah: data.tanggal_pindah
            ? new Date(data.tanggal_pindah).toISOString().split("T")[0]
            : "",
  ...
  const validateForm = () => {
    ...
    if (!formData.tanggal_pindah) return "Tanggal Pindah harus diisi.";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetchData(`/mutasi/updatePindahKeluar/${id}`, {
        method: "PUT",
        data: {
          ...formData,
          tanggal_pindah: formData.tanggal_pindah + "T00:00:00.000Z",
        },
      });
      Swal.fire({
        icon: "success",
        title: "Berhasil",
        text: "Data pindah keluar berhasil diperbarui!",
      });
      router.push("/pindah-keluar");
    } catch (err) {
      Swal.fire({
        icon: "error",
        title: "Gagal",
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl font-semibold text-blue-900">
                    Edit Data Pindah Keluar
                  </h1>
                  <p className="text-sm text-gray-600">
                    Perbarui data pindah keluar
                  </p>
                </div>
              </div>
            </div>
          ...
          <main className="p-6 space-y-6">
            ...
            <Card className="border-0 bg-white/80 shadow-lg">
              <CardHeader>
                ...
                <CardDescription>Perbarui detail data pindah keluar</CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    <Label htmlFor="tanggal_pindah" className="text-gray-600">
                      Tanggal Pindah
                    </Label>
                    <Input
                      id="tanggal_pindah"
                  ...
                  <div>
                    <Label htmlFor="alamat_tujuan" className="text-gray-600">
                      Alamat Tujuan
                    </Label>
                    <Input
                      id="alamat_tujuan"
                  ...
                  <div className="flex justify-end gap-2">
                    <Button
                      ...
                      onClick={() => router.push("/pindah-keluar")}
                    >
                      Batal
                    </Button>
                    <Button
