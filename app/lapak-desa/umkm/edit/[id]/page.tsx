...
export default function EditUMKM() {
  ...
  useEffect(() => {
    const loadUMKM = async () => {
      setIsLoading(true);
      try {
        const response = await fetchData(`/lapak-desa/umkm/${id}`);
        console.log("API Response:", response);
        const data: UMKM = response.umkm || response;
        setNamaUmkm(data.nama_umkm);
        setDeskripsiUmkm(data.deskripsi_umkm || "");
        setKontakWa(data.kontak_wa);
        setExistingFoto(data.foto_umkm || null);
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await fetchData(`/lapak-desa/umkm/${id}`, {
        method: "PUT",
        data: formData,
      });
      Swal.fire({
      ...
      Swal.fire({
        icon: "error",
        title: "Gagal",
        text: `Gagal memperbarui UMKM: ${err.message || "Terjadi kesalahan"}`,
      });
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
            ...
            <div className="flex h-20 items-center justify-between px-6 md:px-10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Perbarui data UMKM</p>
                </div>
              </div>
            </div>
          </header>
          ...
          <main className="p-6">
            <Card className="border-0 bg-white/80 shadow-lg max-w-2xl mx-auto">
              <CardHeader>
                <CardTitle className="text-blue-900">Form Edit UMKM</CardTitle>
                <CardDescription>Perbarui detail UMKM</CardDescription>
              </CardHeader>
              <CardContent>
                {isLoading && (
