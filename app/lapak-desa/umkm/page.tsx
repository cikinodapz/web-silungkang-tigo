...
export default function UMKMListPage() {
  ...
  useEffect(() => {
    const loadUMKM = async () => {
      setIsLoading(true);
      try {
        const response = await fetchData("/lapak-desa/umkm");
  ...
  const handleDeleteUMKM = async (id: string) => {
    const result = await Swal.fire({
      icon: "warning",
      title: "Konfirmasi",
      text: "Apakah Anda yakin ingin menghapus UMKM ini?",
    ...
    setIsLoading(true);
    try {
      await fetchData(`/lapak-desa/umkm/${id}`, { method: "DELETE" });
      setUmkmList(umkmList.filter((record) => record.id !== id));
      if (
        umkmList.length - 1 <= (currentPage - 1) * itemsPerPage &&
        currentPage > 1
      ) {
  ...
  const filteredRecords = umkmList.filter(
    (record) =>
      ...
      record.kontak_wa.toLowerCase().includes(searchQuery.toLowerCase())
  );
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
            ...
            <div className="flex h-20 items-center justify-between px-6 md:px-10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Kelola UMKM desa</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <Input
                    placeholder="Cari nama, deskripsi, atau kontak WA..."
                ...
                <Button
                  ...
                  onClick={() => router.push("/lapak-desa/umkm/tambah")}
                >
          ...
          <main className="p-6 space-y-6">
            ...
            <Card className="border-0 bg-white/80 shadow-lg">
              <CardHeader>
                <CardTitle className="text-blue-900">Daftar UMKM</CardTitle>
                <CardDescription>Kelola UMKM desa</CardDescription>
              </CardHeader>
              <CardContent>
                      ...
                      {currentItems.map((record) => (
                        ...
                        >
                          <td className="px-6 py-4">
                            {record.foto_umkm ? (
                              <img
                                src={getFotoUrl(record.foto_umkm)}
                                alt={record.nama_umkm}
                          ...
                          <td className="px-6 py-4">{record.nama_umkm}</td>
                          <td className="px-6 py-4">{record.deskripsi_umkm}</td>
                          <td className="px-6 py-4">{record.kontak_wa}</td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4 text-right">
                            <div className="flex justify-end gap-2">
                              ...
                              <Button
                                ...
                                onClick={() => handleDeleteUMKM(record.id)}
                              >
