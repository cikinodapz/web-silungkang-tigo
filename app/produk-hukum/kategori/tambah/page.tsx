...
export default function TambahKategoriProdukHukumPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await fetchData("/produk-hukum/createKategoriProdukHukum", {
        method: "POST",
      ...
      Swal.fire({
        icon: "success",
        title: "Berhasil",
        text: "Kategori produk hukum berhasil ditambahkan!",
      });
      router.push("/produk-hukum/kategori");
    } catch (err) {
      setError(`Gagal menambahkan kategori: ${err.message || "Terjadi kesalahan"}`);
      Swal.fire({
        icon: "error",
        title: "Gagal",
        text: `Gagal menambahkan kategori: ${err.message || "Terjadi kesalahan"}`,
      });
    } finally {
      setIsLoading(false);
    }
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl font-semibold text-blue-900">
                    Tambah Kategori Produk Hukum
                  </h1>
                  <p className="text-sm text-gray-600">
                    Tambah kategori baru untuk produk hukum desa
                  </p>
                </div>
              </div>
              <Button
          ...
          <main className="p-6">
            <Card className="border-0 bg-white/80 shadow-lg max-w-2xl mx-auto">
              <CardHeader>
                ...
                <CardDescription>Masukkan detail kategori produk hukum</CardDescription>
              </CardHeader>
              <CardContent>
