...
export default function KategoriProdukHukumListPage() {
  const [kategoriProdukHukumList, setKategoriProdukHukumList] = useState<KategoriProdukHukum[]>([]);
  ...
  useEffect(() => {
    const loadKategoriProdukHukum = async () => {
      ...
      try {
        ...
        setKategoriProdukHukumList(data);
      } catch (err) {
    ...
    loadKategoriProdukHukum();
  }, []);

  const handleDeleteKategoriProdukHukum = async (id: string) => {
    const result = await Swal.fire({
      icon: "warning",
      title: "Konfirmasi",
      text: "Apakah Anda yakin ingin menghapus kategori produk hukum ini?",
    ...
    setIsLoading(true);
    try {
      await fetchData(`/produk-hukum/deleteKategoriProdukHukum/${id}`, { method: "DELETE" });
      setKategoriProdukHukumList(kategoriProdukHukumList.filter((record) => record.id !== id));
      if (
        kategoriProdukHukumList.length - 1 <= (currentPage - 1) * itemsPerPage &&
        currentPage > 1
      ) {
      ...
      Swal.fire({
        icon: "success",
        title: "Berhasil",
        text: "Kategori produk hukum berhasil dihapus!",
      });
    } catch (err) {
      Swal.fire({
        icon: "error",
  ...
  const filteredRecords = kategoriProdukHukumList.filter((record) =>
            ...
            <div className="flex h-20 items-center justify-between px-6 md:px-10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl font-semibold text-blue-900">
                    Data Kategori Produk Hukum
                  </h1>
                  <p className="text-sm text-gray-600">
                    Kelola kategori produk hukum desa
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <Button
                  ...
                  onClick={() => router.push("/produk-hukum/kategori/tambah")}
                >
          ...
          <main className="p-6 space-y-6">
            ...
            {!isLoading && filteredRecords.length === 0 && !error && (
              <div className="text-center text-gray-600">
                Tidak ada data kategori produk hukum.
              </div>
            )}
            ...
            <Card className="border-0 bg-white/80 shadow-lg">
              <CardHeader>
                <CardTitle className="text-blue-900">
                  Daftar Kategori Produk Hukum
                </CardTitle>
                <CardDescription>Kelola kategori produk hukum</CardDescription>
              </CardHeader>
              <CardContent>
                          ...
                          <td className="px-4 py-3 text-right">
                            <div className="flex justify-end gap-2">
                              <Button
                                ...
                                onClick={() =>
                                  router.push(`/produk-hukum/kategori/edit/${record.id}`)
                                }
                              >
                              ...
                              <Button
                                ...
                                onClick={() => handleDeleteKategoriProdukHukum(record.id)}
                              >
