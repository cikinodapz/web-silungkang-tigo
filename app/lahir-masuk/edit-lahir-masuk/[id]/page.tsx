...
export default function EditLahirMasukPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Load existing LahirMasuk data
        const response = await fetchData(`/mutasi/getLahirMasuk/${id}`);
        const data = response.lahirMasuk || response;
        setFormData({
          nama: data.nama || "",
          nik: data.nik || "",
          tanggal_lahir_masuk: data.tanggal_lahir_masuk
            ? new Date(data.tanggal_lahir_masuk).toISOString().split("T")[0]
            : "",
  ...
  const validateForm = () => {
    ...
    if (!formData.tanggal_lahir_masuk) return "Tanggal Lahir/Masuk harus diisi.";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetchData(`/mutasi/updateLahirMasuk/${id}`, {
        method: "PUT",
        data: {
          ...formData,
          tanggal_lahir_masuk: formData.tanggal_lahir_masuk + "T00:00:00.000Z",
        },
      });
      Swal.fire({
        icon: "success",
        title: "Berhasil",
        text: "Data Lahir/Masuk berhasil diperbarui!",
      });
      router.push("/lahir-masuk");
    } catch (err) {
      Swal.fire({
        icon: "error",
        title: "Gagal",
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl font-semibold text-blue-900">
                    Edit Data Lahir/Masuk
                  </h1>
                  <p className="text-sm text-gray-600">
                    Perbarui data kelahiran atau pendatang
                  </p>
                </div>
              </div>
            </div>
          ...
          <main className="p-6 space-y-6">
            ...
            <Card className="border-0 bg-white/80 shadow-lg">
              <CardHeader>
                ...
                <CardDescription>Perbarui detail data Lahir/Masuk</CardDescription>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    <Label htmlFor="tanggal_lahir_masuk" className="text-gray-600">
                      Tanggal Lahir/Masuk
                    </Label>
                    <Input
                      id="tanggal_lahir_masuk"
                      name="tanggal_lahir_masuk"
                      type="date"
                      value={formData.tanggal_lahir_masuk}
                  ...
                  <div>
                    <Label htmlFor="alamat_sebelumnya" className="text-gray-600">
                      Alamat Sebelumnya
                    </Label>
                    <Input
                      id="alamat_sebelumnya"
                      name="alamat_sebelumnya"
                      value={formData.alamat_sebelumnya}
                      onChange={handleChange}
                      placeholder="Masukkan alamat sebelumnya (opsional)"
                      className="border-gray-300 focus:border-blue-500 focus:ring-blue-500"
                    />
                  </div>
                  <div>
                  ...
                  <div className="flex justify-end gap-2">
                    <Button
                      ...
                      onClick={() => router.push("/lahir-masuk")}
                    >
                      Batal
                    </Button>
                    <Button
