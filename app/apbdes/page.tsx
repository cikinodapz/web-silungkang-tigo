...
export default function APBDesListPage() {
  ...
  useEffect(() => {
    const loadAPBDes = async () => {
      setIsLoading(true);
      try {
        const response = await fetchData("/apbdes/getAllAPBDes");
  ...
  const handleDeleteAPBDes = async (id: string) => {
    const result = await Swal.fire({
      icon: "warning",
      title: "Konfirmasi",
      text: "Apakah Anda yakin ingin menghapus data APBDes ini?",
    ...
    setIsLoading(true);
    try {
      await fetchData(`/apbdes/deleteAPBDes/${id}`, { method: "DELETE" });
      setApbdesList(apbdesList.filter((record) => record.id !== id));
      if (
        apbdesList.length - 1 <= (currentPage - 1) * itemsPerPage &&
        currentPage > 1
      ) {
  ...
  const filteredRecords = apbdesList.filter(
    (record) =>
      record.pendanaan.toLowerCase().includes(searchQuery.toLowerCase()) ||
      record.jenis_apbd.toLowerCase().includes(searchQuery.toLowerCase()) ||
      record.keterangan?.toLowerCase().includes(searchQuery.toLowerCase()) ||
      record.tahun.toString().includes(searchQuery)
  );
            ...
            <div className="flex h-20 items-center justify-between px-6 md:px-10">
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  ...
                  <Input
                    placeholder="Cari pendanaan, jenis, atau tahun..."
                ...
                <Button
                  ...
                  onClick={() => router.push("/apbdes/tambah")}
                >
          ...
          <main className="p-6 space-y-6">
            ...
            <Card className="border-0 bg-white/80 shadow-lg">
              <CardHeader>
                ...
                <CardDescription>Kelola data anggaran desa</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm text-gray-600">
                    <thead className="bg-blue-50 text-blue-900">
                      <tr>
                        <th className="px-4 py-3 font-semibold">Pendanaan</th>
                        <th className="px-4 py-3 font-semibold">Jumlah Dana (Rp)</th>
                        <th className="px-4 py-3 font-semibold">Jenis APBDes</th>
                    ...
                    <tbody>
                      {currentItems.map((record) => (
                        ...
                        >
                          <td className="px-4 py-3">{record.pendanaan}</td>
                          <td className="px-4 py-3">
                            {record.jumlah_dana.toLocaleString("id-ID")}
                          </td>
                          <td className="px-4 py-3">{record.jenis_apbd}</td>
                          <td className="px-4 py-3">{record.tahun}</td>
                          ...
                          <td className="px-4 py-3 text-right">
                            <div className="flex justify-end gap-2">
                              <Button
                                ...
                                onClick={() =>
                                  router.push(`/apbdes/edit/${record.id}`)
                                }
                              >
                              ...
                              <Button
                                ...
                                onClick={() => handleDeleteAPBDes(record.id)}
                              >
