...
export default function EditAPBDesPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true);
      try {
        const response = await fetchData(`/apbdes/getAPBDes/${id}`);
        const data = response.apbdes || response;
        setFormData({
          pendanaan: data.pendanaan || "",
          jumlah_dana: data.jumlah_dana?.toString() || "",
          jenis_apbd: data.jenis_apbd || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetchData(`/apbdes/editAPBDes/${id}`, {
        method: "PUT",
        data: {
          ...formData,
      ...
      Swal.fire({
        icon: "success",
        title: "Berhasil",
        text: "Data APBDes berhasil diperbarui!",
      });
      router.push("/apbdes");
    } catch (err) {
      Swal.fire({
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    Perbarui data anggaran pendapatan dan belanja desa
                  </p>
                </div>
              </div>
            </div>
          ...
          <main className="p-6 space-y-6">
            ...
            <Card className="border-0 bg-white/80 shadow-lg">
              <CardHeader>
                ...
                <CardDescription>Perbarui detail data APBDes</CardDescription>
              </CardHeader>
              <CardContent>
